"""C: Conv-coded DS-CDMA in AWGN and Rician fading (variant)."""
...
def rician_gain(n: int) -> np.ndarray:
    los = np.sqrt(K_FACTOR / (K_FACTOR + 1.0))
    sc = np.sqrt(1.0 / (2.0 * (K_FACTOR + 1.0)))
    return np.abs(los + sc * (rng.standard_normal(n) + 1j * rng.standard_normal(n)))
...
def simulate(channel: str) -> np.ndarray:
    ...
    for i, db in enumerate(EBN0_DB):
        ...
        for _ in range(trials_for(int(db))):
            ...
            h = rician_gain(sym.size) if channel == "rician" else np.ones(sym.size)
...
ber_ric = simulate("rician")

plt.figure(figsize=(8, 5))
plt.semilogy(EBN0_DB, ber_awgn, "o-", linewidth=2, markersize=6, label="AWGN")
plt.semilogy(
    EBN0_DB, ber_ric, "s-", linewidth=2, markersize=6, label=f"Rician (K={K_FACTOR:g})"
)
plt.grid(True, which="both", ls="--", alpha=0.6)
plt.xlabel("Eb/N0 (dB)")
plt.ylabel("Bit Error Rate (BER)")
plt.title("Conv-Coded DS-CDMA: AWGN vs Rician (Variant)")
plt.legend()
plt.tight_layout()
plt.show(block=True)
